...
static void tpms_udp_begin(int port) {
    ...
    int f = fcntl(tpms_sock, F_GETFL, 0);
    fcntl(tpms_sock, F_SETFL, f | O_NONBLOCK);
    int r=1; ::setsockopt(tpms_sock,SOL_SOCKET,SO_REUSEADDR,&r,sizeof(r));

    struct sockaddr_in srv{};
    srv.sin_family=AF_INET; srv.sin_port=htons((uint16_t)port);
    srv.sin_addr.s_addr=INADDR_ANY;
    if (::bind(tpms_sock,(struct sockaddr*)&srv,sizeof(srv))<0) {
        ESP_LOGE("tpms_udp","bind err %d",errno);
        ::close(tpms_sock); tpms_sock=-1; return;
    }
    ESP_LOGI("tpms_udp","Socket UDP ouvert port %d",port);
}
...
{
    if (tpms_sock < 0) return;
    char buf[128]; struct sockaddr_in src{}; socklen_t sl=sizeof(src); int n;
    while ((n=::recvfrom(tpms_sock,buf,sizeof(buf)-1,0,
                         (struct sockaddr*)&src,&sl))>0) {
        buf[n]='\0';
        if (strncmp(buf,"TPMS2:",6)!=0) { ESP_LOGW("tpms_udp","fmt: %s",buf); continue; }
        float fp=-1,ft=-1,rp=-1,rt=-1,lp=-1,lt=-1,bp=-1,bt=-1;
        if (sscanf(buf+6,"FL=%f/%f,FR=%f/%f,RL=%f/%f,RR=%f/%f",
            ...
            *g_ready=true;
            ...
            ESP_LOGW("tpms_udp","parse err: %s",buf);
        }
    }
}
